@extends('layouts/main')

@section('content')
{{-- <h1 class="mb-3 text-center">{{ $title }}</h1> --}}

<div class="row justify-content-center">
	<div class="col-md-6">
		<form action="/posts">
		@if (request('category'))
			<input type="hidden" name="category" value="{{ request('category') }}">
		@endif
		@if (request('news'))
			<input type="hidden" name="news" value="{{ request('news') }}">
		@endif
		@if (request('author'))
			<input type="hidden" name="author" value="{{ request('author') }}">
		@endif
		@if (request('tags'))
			<input type="hidden" name="tags" value="{{ request('tags') }}">
		@endif
			<div class="input-group mb-3">
			  {{-- <input type="text" class="form-control" placeholder="Search.." name="search" value="{{ request('search') }}">
			  <button class="btn btn-danger" type="submit" >Search</button> --}}
			</div>
		</form>
	</div>
</div>


@if ($posts->count())

<div class="card mb-3">
	@if($posts[0]->image)
    <div style="max-heigt: 350px; overflow:hidden;">
      <img src="{{ asset('storage/' .$posts[0]->image) }}}}" class="img-fluid" alt="{{ $posts[0]->category->name }}">
    </div>
    @elseif ($posts[0]->category_id != 0)
		<img src="https://source.unsplash.com/800x400?{{ $posts[0]->category->name }}" class="card-img-top" alt="{{ $posts[0]->category->name }}">	
    @elseif ($posts[0]->news_id != 0)
		<img src="https://source.unsplash.com/800x400?{{ $posts[0]->news->name }}" class="card-img-top" alt="{{ $posts[0]->news->name }}">	
    @endif
  <div class="card-body text-center">
    <h3 class="card-title"> <a href="/posts/{{ $posts[0]->slug }}" class="text-decoration-none text-dark"> {{ $posts[0]->title }} </a></h3>
    <small class="text-muted">
		<p>By. <a href="/posts?author={{ $posts[0]->author->username }}" class="text-decoration-none">{{ $posts[0]->author->name }} </a> in 
			@if ($posts[0]->category_id != 0)
				<a href="/posts?category={{ $posts[0]->category->slug }}" class="text-decoration-none">{{ $posts[0]->category->name }} </a>
			@elseif($posts[0]->news_id != 0)
				<a href="/posts?category={{ $posts[0]->news->slug }}" class="text-decoration-none">{{ $posts[0]->news->name }} </a>
			@endif
			{{ $posts[0]->created_at->diffForHumans() }}</p>			
    </small>
    <p class="card-text">{{ $posts[0]->excerpt }}</p>

     <a href="/posts/{{ $posts[0]->slug }}" class="text-decoration-none btn btn-primary">Read More</a>
    
  </div>
</div>

<div class="container">
	<div class="row">
	@foreach ($posts->skip(1) as $post)
		<div class="col-md-4 mb-3">
			<div class="card">
				@if ($post->category_id != 0)
				<div class="position-absolute bg-dark px-3 py-2" style="background-color: rgba(0,0,0,0.7);"><a href="/posts?category={{ $post->category->slug }}" class="text-white text-decoration-none">{{ $post->category->name }} </a></div>
				@elseif($post->news_id != 0)
				<div class="position-absolute bg-dark px-3 py-2" style="background-color: rgba(0,0,0,0.7);"><a href="/posts?category={{ $post->news->slug }}" class="text-white text-decoration-none">{{ $post->news->name }} </a></div>
				@endif
			@if($post->image)
                	<img src="{{ asset('storage/' .$post->image) }}}}" class="img-fluid" alt="{{ $post->category->name }}">
            @elseif($post->category_id != 0)
					<img src="https://source.unsplash.com/500x400?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}">
			@elseif($post->news_id != 0)
					<img src="https://source.unsplash.com/500x400?{{ $post->news->name }}" class="card-img-top" alt="{{ $post->news->name }}">
            @endif
			  <div class="card-body">
			    <h5 class="card-title">{{ $post->title }}</h5>
  				<small class="text-muted">
			    <p>By. <a href="/posts?author={{ $post->author->username }}" class="text-decoration-none">{{ $post->author->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}</p>
			    </small>
			    <p class="card-text">{{ $post->excerpt }}</p>
			    <a href="/posts/{{ $post->slug }}" class="btn btn-primary">Read More</a>
			  </div>
			</div>
		</div>
		@endforeach
	</div>
</div>

@else
<p class="text-center fs-4">No Posts Found.</p>
@endif

<div class="d-flex justify-content-center">
	{{ $posts->links()}}
</div>
@endsection